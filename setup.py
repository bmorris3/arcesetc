...
PACKAGENAME = metadata.get('package_name', 'arcesetc')
DESCRIPTION = metadata.get('description', 'Exposure time calculator for APO/ARCES')
AUTHOR = metadata.get('author', 'Brett Morris &amp; Trevor Dorn-Wallenstein')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'https://github.com/bmorris3/ARCES-ETC')
__minimum_python_version__ = metadata.get("minimum_python_version", "3.5")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: arcesetc requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
